...
-- Revocar público
REVOKE ALL ON FUNCTION bck.fn_dispatch_background_jobs(uuid) FROM PUBLIC;

-- Dar permisos al rol que ejecutará los workers
GRANT EXECUTE ON FUNCTION bck.fn_dispatch_background_jobs(uuid) TO PUBLIC;

-- Seguridad adicional: Forzar search_path para evitar ataques de shadowing
ALTER FUNCTION bck.fn_dispatch_background_jobs(uuid) SET search_path TO bck, public, pg_temp;
