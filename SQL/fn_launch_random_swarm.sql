...
-- Revocar público
REVOKE ALL ON FUNCTION bck.fn_launch_random_swarm(uuid) FROM PUBLIC;

-- Dar permisos al rol que ejecutará los workers
GRANT EXECUTE ON FUNCTION bck.fn_launch_random_swarm(uuid) TO PUBLIC;

-- Seguridad adicional: Forzar search_path para evitar ataques de shadowing
ALTER FUNCTION bck.fn_launch_random_swarm(uuid) SET search_path TO bck, public, pg_temp;
...
-- Revocar público
REVOKE ALL ON FUNCTION bck.run_task_random(uuid) FROM PUBLIC;

-- Dar permisos al rol que ejecutará los workers
GRANT EXECUTE ON FUNCTION bck.run_task_random(uuid) TO PUBLIC;

-- Seguridad adicional: Forzar search_path para evitar ataques de shadowing
ALTER FUNCTION bck.run_task_random(uuid) SET search_path TO bck, public, pg_temp;
