...
-- Revocar público
REVOKE ALL ON FUNCTION bck.fn_launch_sequential_chain(uuid) FROM PUBLIC;

-- Dar permisos al rol que ejecutará los workers
GRANT EXECUTE ON FUNCTION bck.fn_launch_sequential_chain(uuid) TO PUBLIC;

-- Seguridad adicional: Forzar search_path para evitar ataques de shadowing
ALTER FUNCTION bck.fn_launch_sequential_chain(uuid) SET search_path TO bck, public, pg_temp;
...
-- Revocar público
REVOKE ALL ON FUNCTION bck.run_task_sequential(uuid) FROM PUBLIC;

-- Dar permisos al rol que ejecutará los workers
GRANT EXECUTE ON FUNCTION bck.run_task_sequential(uuid) TO PUBLIC;

-- Seguridad adicional: Forzar search_path para evitar ataques de shadowing
ALTER FUNCTION bck.run_task_sequential(uuid) SET search_path TO bck, public, pg_temp;
